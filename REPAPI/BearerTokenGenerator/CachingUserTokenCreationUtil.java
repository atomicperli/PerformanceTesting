...
public class CachingUserTokenCreationUtil {
        ...
        private static final String CACHED_TOKEN_DIRECTORY = "cached_tokens";
        ...
        public static void main(String[] args) {
                ...
                CachingUserTokenCreationUtil tokenCreation = new CachingUserTokenCreationUtil();

                String bearerToken = tokenCreation.getPersistedToken(username);
                if (bearerToken == null) {
                    bearerToken = tokenCreation.getBearerToken(username, password);
                    tokenCreation.persistToken(username, bearerToken);
                }
        ...
        /**
         * Persists a token to a file. Filename is generated from username.
         */
        public void persistToken(String username, String token) {
        	File persistedTokenFile = getTokenFile(username);
         ...
         */
        public String getPersistedToken(String username) {

        	File persistedTokenFile = getTokenFile(username);
        	
        	if (!persistedTokenFile.exists()) {
        		return null;
        	}
        ...
        public CachingUserTokenCreationUtil() {
        ...
        /**
         * Generates the token File object under the classpath root directory.
         * @return
         */
		private File getTokenFile(String username) {
			String basePathOfClass = getClass().getProtectionDomain().getCodeSource().getLocation().getFile();
        	basePathOfClass = basePathOfClass.replaceAll("%20", "\\ "); // Escape spaces.
        	File tokensDir = new File (basePathOfClass, CACHED_TOKEN_DIRECTORY);
        	if (!tokensDir.exists()) {
        		tokensDir.mkdir();
        	}
        	// System.out.println("userName1:" + username);	
        	// Condition the username so it can be used for a filename.
        	String tokenFileName =  username.replaceAll("[\\\\/:*?\"<>|]", "_");
        	tokenFileName =  tokenFileName.replaceAll("@", "_at_");
        	tokenFileName =  tokenFileName.replaceAll("\\+", "_plus_");
        	
        	// System.out.println("tokenFileName:" + tokenFileName);
       	
        	File persistedTokenFile = new File(tokensDir, tokenFileName);
			return persistedTokenFile;
		}        
